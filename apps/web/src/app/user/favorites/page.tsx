    ...
    const fetchData = async () => {
      try {
        ...
        if (favRes.status === 'fulfilled') {
          const favPayload = favRes.value.data?.data || favRes.value.data;
          const favItems = Array.isArray(favPayload) ? favPayload : (favPayload?.data || []);
          setFavorites(favItems);
        }

        if (recentRes.status === 'fulfilled') {
          const recentPayload = recentRes.value.data?.data || recentRes.value.data;
          const recentItems = Array.isArray(recentPayload) ? recentPayload : (recentPayload?.data || []);
          setRecentEscorts(recentItems);
        }
      } catch {
        /* silently */
      } finally {
        ...
        <div className="mt-6 space-y-3">
          {favorites.length === 0 ? (
            ...
            favorites.map((fav) => (
              ...
              >
                {/* Avatar */}
                <Link href={`/user/escorts/${fav.escort.id}`} className="flex-shrink-0">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <Link
                      href={`/user/escorts/${fav.escort.id}`}
                  ...
                  {fav.escort.escortProfile && (
                    <div className="mt-1 flex items-center gap-3 text-xs text-dark-400">
                      <span className="flex items-center gap-1">
                        <Star className="h-4 w-4 inline-block" /> {fav.escort.escortProfile.ratingAvg.toFixed(1)}
                      </span>
                      <span>{fav.escort.escortProfile.totalBookings.toLocaleString('id-ID')} booking</span>
                ...
                <div className="flex items-center gap-2">
                  <Link
                    href={`/user/bookings/new?escortId=${fav.escort.id}`}
          ...
          {recentEscorts.length === 0 ? (
            ...
            <>
              ...
              {recentEscorts.map((item) => (
                ...
                >
                  {/* Avatar */}
                  <Link href={`/user/escorts/${item.escort.id}`} className="flex-shrink-0">
                    <div className="h-16 w-16 overflow-hidden rounded-xl bg-dark-700">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <Link
                        href={`/user/escorts/${item.escort.id}`}
                        className="font-medium text-dark-100 hover:text-brand-400"
                      >
                  ...
                  {/* Actions */}
                  <Link
                    href={`/user/bookings/new?escortId=${item.escort.id}&serviceType=${item.lastBooking.serviceType}`}
