...
const STATUS_CONFIG: Record<PaymentStatus, { icon: string; title: string; desc: string; color: string }> = {
  PENDING: {
    ...
    desc: 'Pembayaran sedang diproses. Mohon tunggu sebentar.',
...
function PaymentStatusContent() {
  ...
  // For mock mode, trigger webhook to settle payment
  useEffect(() => {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="mt-4 text-sm text-dark-400">
          {isMock ? 'Memproses pembayaran mock...' : 'Memuat status pembayaran...'}
        </p>
      </div>
    );
  }
          ...
          <div className="py-8 text-center">
            ...
            {isMock && mockSettled && (
              <div className="mb-4 rounded-lg border border-green-500/20 bg-green-500/10 px-3 py-1.5 text-xs text-green-400 inline-block">
                <FlaskConical className="h-4 w-4 inline-block" /> Mode Simulasi — Pembayaran otomatis diproses
              </div>
            )}
            ...
            <div className="mt-8 rounded-xl border border-dark-700/50 bg-dark-800/30 p-4 text-left">
              <div className="space-y-3 text-sm">
                ...
                {payment.paymentType === 'DP_50' && (
                  <div className="flex justify-between">
                    <span className="text-dark-500">Tipe</span>
                    <span className="text-yellow-400 text-xs">DP 50%</span>
                  </div>
                )}
                <div className="flex justify-between">
            ...
            {payment.status === 'PENDING' && (
              <div className="mt-6 rounded-lg border border-yellow-500/20 bg-yellow-500/5 p-3">
                <p className="text-xs text-yellow-400">
                  <AlarmClock className="h-4 w-4 inline-block" /> Pembayaran masih dalam proses. Halaman ini akan otomatis diperbarui.
                </p>
              </div>
            )}
            ...
            {payment.status === 'ESCROW' && (
              <div className="mt-6 rounded-lg border border-green-500/20 bg-green-500/5 p-3">
                <p className="text-xs text-green-400">
                  <CheckCircle2 className="h-4 w-4 inline-block" /> Pembayaran telah dikonfirmasi! Anda dapat melanjutkan ke proses check-in.
                </p>
              </div>
            )}
            ...
            <div className="mt-8 flex flex-col gap-3">
              ...
              >
                {payment.status === 'ESCROW' ? 'Lanjut ke Detail Booking' : 'Lihat Detail Booking'}
              </Button>
