...
export default function ChatRoomPage() {
  ...
  useEffect(() => {
    ...
    // Poll every 5s for new messages (will be replaced with WebSocket)
  ...
  // Check partner online status periodically
  useEffect(() => {
    const checkOnline = async () => {
      if (!partner) return;
      try {
        const res = await api.get(`/chats/${bookingId}/online-status`);
        setIsPartnerOnline(res.data?.partnerOnline || false);
      } catch {
  ...
  const handleTyping = useCallback(() => {
    ...
    // TODO: Replace with Socket.io emit when integrated
